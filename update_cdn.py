...
def fetch_mrz_data() -> Optional[List[Dict]]:
    ...
    headers = {}
    if os.path.exists(ETAG_FILE) and os.path.exists(
            os.path.join(SCRIPT_DIR, "connect.json")):
...
def calculate_connect_metadata(sections: List[Dict]) -> Dict:
    ...
    print(
        f"  - Final exams: {
            metadata['finalExamStartDate']} to {
            metadata['finalExamEndDate']}")

    return metadata


def generate_connect_json(
        sections: List[Dict],
        output_path: str = "connect.json"):
    ...
    print(
        f"✓ connect_metadata.json created successfully ({
            metadata_size:.2f} KB)")
...
def manage_current_backup(metadata: Dict, sections: List[Dict]):
    ...
    if curr_backups:
        ...
        print(
            f"\nFound existing current backup: {
                os.path.basename(old_curr_backup)}")
...
def generate_exams_json(sections: List[Dict], output_path: str = "exams.json"):
    ...
    print(
        f"  Regular: {
            regular_size:.1f} KB | Gzipped: {
            gzip_size:.1f} KB (saved {
                compression_ratio:.1f}%)")
    print(f"  Total exams: {metadata['totalExams']}")
    print(
        f"  Mid exams: {
            metadata['midExamStartDate']} to {
            metadata['midExamEndDate']}")
    print(
        f"  Final exams: {
            metadata['finalExamStartDate']} to {
            metadata['finalExamEndDate']}")
...
def main():
    ...
    try:
        ...
        if sections is None:
            print("\n✓ Data has not changed since last update.")
